import React from "react";

export default function AnalyticsIcon() {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M13.8333 0.5H2.16667C1.25 0.5 0.5 1.25 0.5 2.16667V13.8333C0.5 14.75 1.25 15.5 2.16667 15.5H13.8333C14.75 15.5 15.5 14.75 15.5 13.8333V2.16667C15.5 1.25 14.75 0.5 13.8333 0.5ZM13.8333 13.8333H2.16667V2.16667H13.8333V13.8333Z"
        fill="#4B4B4B"
      />
      <path d="M5.5 8H3.83333V12.1667H5.5V8Z" fill="#4B4B4B" />
      <path d="M12.1667 3.83333H10.5V12.1667H12.1667V3.83333Z" fill="#4B4B4B" />
      <path
        d="M8.83333 9.66667H7.16667V12.1667H8.83333V9.66667Z"
        fill="#4B4B4B"
      />
      <path d="M8.83333 6.33333H7.16667V8H8.83333V6.33333Z" fill="#4B4B4B" />
    </svg>
  );
}
